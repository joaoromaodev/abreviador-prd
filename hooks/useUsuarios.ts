...
export function useUsuarios() {
  ...
  async function adicionar(dados: { email: string; nome: string; papel: "master" | "admin" | "usuario"; setores: string[] }) {
    const resposta = await fetch("/api/usuarios", {
      method: "POST",
  ...
  async function editar(email: string, dados: { nome: string; papel: "master" | "admin" | "usuario"; setores: string[] }) {
